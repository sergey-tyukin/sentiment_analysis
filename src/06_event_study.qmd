...
```{r}
# Находим события на неторговых днях (где ret = NA)
...
# Функция поиска следующего торгового деня
find_next_trade_date <- function(target_date, ticker, data) {
  trade_dates <- data |>
    filter(ticker == .env$ticker, !is.na(ret), !is.na(market_return)) |>
    pull(date) |>
...
# Сдвигаем даты на следующий торговый день
...
# Удаляем старые события из es_data
es_data <- es_data |>
  filter(!(is_event == TRUE & is.na(ret)))

# Добавляем сдвинутые события в es_data
# Находим строки с новыми датами и заполняем поля событий
...
# Проверка: событий на неторговых днях больше нет
check <- es_data |>
  filter(is_event == TRUE, is.na(ret)) |>
  nrow()
